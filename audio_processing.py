...
def _plot_spectrogram(
    ...
    ylabel="freq_bin",
    ax=None,
    file_name="images/spectrogram.png",
    num_channels=1,
):
        ...
        for index in range(len(df)):
            try:
                ...
                output_dir_path = os.path.dirname(
                    os.path.realpath(processed_audio_out_file)
                )
                # check if file exists
                if os.path.isfile(processed_audio_out_file):
                    continue
                # check if directory exists
                elif not os.path.exists(output_dir_path) or not os.path.isdir(
                    output_dir_path
                ):
        ...
        for index in range(len(df)):
            try:
                ...
                sgram = AudioUtil.spectro_gram(
                    dur_aud, n_mels=128, n_fft=512, hop_len=None
                )
                ...
                output_dir_path = os.path.dirname(
                    os.path.realpath(spectrogram_file_name)
                )
                # check if file exists
                ...
                _plot_spectrogram(
                    sgram, file_name=spectrogram_file_name, num_channels=channel
                )
    ...
    for metadata_file in metadata_files:
        ...
        for index in range(len(df)):
            try:
                ...
                _plot_waveform(
                    waveform=y_.cpu().detach(),
                    sr=sample_rate,
                    file_name=waveform_file_name,
                    title=f"ClassID: {df.loc[index, 'classID']}",
